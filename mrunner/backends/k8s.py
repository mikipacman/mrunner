...
class Job(client.V1Job):
    ...
    def __init__(self, image, experiment):
        ...
        node_selector = {"load": "on-demand"}
        tolerations = [client.V1Toleration(key="reserved-pool", operator="Equal", value="true", effect="NoSchedule")]
        pod_spec = client.V1PodSpec(restart_policy='Never', containers=[ctr],
                                    node_selector=node_selector, tolerations=tolerations)#,volumes=[vol])
